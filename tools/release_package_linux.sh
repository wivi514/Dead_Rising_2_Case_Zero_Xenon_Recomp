...
# 26 must be present: a partial set would generate a bank with some pad glyphs
...
[ "$CHIPS" -eq 26 ] || fail "tools/release/kbm_chips has $CHIPS of 26 chip blobs"
mkdir -p "$STAGE/kbm_chips"
cp "$ROOT/tools/release/kbm_chips"/*.dxt "$STAGE/kbm_chips/"
echo "    kbm_chips/                      26 key-cap prompt icons (our art)"
