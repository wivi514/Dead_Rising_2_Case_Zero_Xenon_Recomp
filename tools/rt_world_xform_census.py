...
def main():
    ...
    with open(out, 'w') as f:
        f.write('{\n')
        f.write(' "_comment": "Generated by tools/rt_world_xform_census.py. Each '
                'entry names the VS constant rows carrying that shader\'s '
                'object->world transform, INNERMOST STAGE FIRST, as kind@base pairs. '
                '`direct` is a row-major 4x3 at vc(base..base+2); `palette` is a blend '
                'of vc(base + 3k) entries by per-vertex weights. A palette stage also '
                'carries a blend field, slot:strideDwords:weightOffsetDw:'
                'indexOffsetDw:bytes, naming the vertex-buffer layout the weights and '
                'matrix indices arrive in, with one byte digit per influence IN THE '
                'ORDER THE SHADER APPLIES THEM. Regenerate after any change to the '
                'shader bank.",\n')
        f.write(' "shaders": {\n')
        ...
        f.write('}\n')
    # A SELF-CHECK ON THE WRITER, because this file is written by string formatting and
    # read by a deliberately small string-search parser in the runtime — neither of which
    # would notice a stray quote. The census tools use a real JSON parser and DO notice,
    # by returning an empty table, which presents as "the transform table has no entries"
    # rather than as a broken file. That happened once; one line stops it happening again.
    try:
        json.loads(open(out).read())
    except Exception as e:
        print('\nWROTE INVALID JSON: %s' % e, file=sys.stderr)
        return 2
