...
- `docs/` — the project's memory. **Read in this order for a new session:**
  ...
  - **THE LIVE HAND-OFF IS ALWAYS THE HIGHEST-NUMBERED `partNN-kickoff.md`**, and it
    ...
    **AS OF 2026-09-14 THE OPERATOR'S SIX F9 REPORTS (`~/XenonLive/Player Issues/`)
    ...
    `docs/lighting-plan-part120.md`**~~ — **PART 120 (2026-09-16, early morning) RAN IT
    AND NAMED THE TERM: THE TITLE'S AUTO-EXPOSURE READS ITS 1x1 LUMINANCE RESOLVE BACK
    ON THE CPU (`sub_825D65A8` -> `sub_825D6D18`, the same formula as DR2 PC's
    `LuminanceToExposure.bcp`), THIS RENDERER NEVER WROTE A RESOLVE INTO GUEST MEMORY,
    AND THE OPERATOR'S XENIA RUNS `readback_resolve = "none"` — so both emulators fed
    the controller its `lum == 0 -> 1.0` sentinel and the exposure sat on the lighting
    table's MINIMUM at every hour (`prologue.csv`: night min 0.1 / max 1.5). Tiny
    ...
    **0 of 302**, with the 3072 px the fill already covered **100.0000% byte-identical**
    ...
    saturated thread 29% of the time, a migration on every wake); `CZ_GUEST_PIN` mode 2
    (default from 8 physical cores with SMT, `=0` the control) pins the Main Thread, the
    Draw Thread, `cz-pump` and `cz-draw` each to its own core, sibling empty — `cz-draw`
    −0.52, Main −0.35, Draw −0.58 ms/frame monotone, p99 12 -> 11; the wall MEDIAN is
    quantised at 1 ms by the vblank and cannot read it (gotcha 572). Havok 4 workers:
    killed (waits +0.5 > CPU −0.21). `docs/perf-plan-part118.md` §5 the honest answer;
    gotchas 570-573.** ~~It was
    `part117-kickoff.md` — PART 117 (the afternoon of 2026-09-12, unattended) PUT THE
    PUMP ON TWO CORES: the PM4 walk stays on `cz-pump`, every renderer call and every
    guest-visible store executes in stream order on `cz-draw` from one ordered stream
    (`gpu/pump_split.{h,cpp}`); the PMU had first shown the pump to be INSTRUCTION-bound
    (IPC 1.6, 0.7 GB/s from DRAM), not the byte-bound thread part 111 named (gotcha 551
    retracted in part); measured −1.19 ms median, monotone, three runs a side (~96 -> ~110
    fps at the crowd), ON by default from six physical cores; the parked wait-any wake
    follows it; THE FRAME'S BOUND IS NOW THE GUEST'S MAIN THREAD (8.1-8.4 ms CPU).
    `docs/perf-plan-part117.md` §0/§4 the record; gotchas 565-569.~~ ~~It was
    `part116-kickoff.md` — PART 116 (the night of 2026-09-11/12, unattended) RAN
    `docs/perf-plan-part116.md` END TO END AND ITS §4 IS THE RECORD: the guest's 8.8 ms
    ...
    guest no longer forbids 120 fps CPU-side, the pump (10.1 ms, byte-bound) does; item
    B's ceiling is now ~137 fps, not 113. Gotchas 559-564. v1.1.0 (co-op, overlay,
    launcher-only online, per-profile saves) is still a DRAFT release awaiting the
    operator's publish.~~ ~~It was `part115-kickoff.md`, and the live work as of
    2026-09-12 was `docs/perf-plan-part116.md` — the operator's 12-hour unattended order.~~
    — CO-OP IS ON MASTER AND THE RELEASE CARRIES IT (co-op plan
    ...
    coverage line could never have caught it (gotcha 547, retracted in place). The
    standing check is `tools/phase_vs_perf.py` — run it once a part beside the other
    gates; its positive control flags `streams` at 53.7x and it found a SECOND lie,
    `textures` under-reporting its own subsystem by 2.1x. (B) THE FOUR IDLE CORES ARE
    REAL AND SO IS A CEILING NOBODY KNEW ABOUT: `CZ_VK_NO_DODRAW=1` measured the pump's
    serial floor at **2.49 ms** against a 10.93 ms baseline, so `F + M/3` = **5.33 ms**
    and the pre-registered 8.0 ms kill DOES NOT FIRE — **but deleting the whole per-draw
    renderer moved the WALL only 11.07 -> 8.80 ms**, because the title's OWN recompiled
    code costs 8.8 ms a frame on two threads and cutting our pump tick 4x and 10x is a
    dead null. **So item B is worth ~2.3 ms and ~90 -> ~113 fps, and 120 fps CPU-side is
    NOT REACHABLE at this crowd.** Whether to build it is the operator's call
...
paste-ready Release body with both SHA-256s. **AND AS OF THE SAME EVENING: v1.0.0 IS
PUBLISHED AND THE REPO IS PUBLIC —
https://github.com/wivi514/Dead_Rising_2_Case_Zero_Xenon_Recomp — THE RELEASE
PROGRAMME IS COMPLETE (the plan's §7 is the closing record).** The publication
followed a same-day fix round the operator drove by playing (device-following prompt
wording — MASH keyboard-only; always-on mouse camera, toggle retired; three Case West
back-imports: XMA hardware loops, LRU texture-slot recycling, the Q glyph — plan §6
addenda 1-5), then a player-first rewrite of both READMEs and the notes. Binaries are
commit 407eb79; the public downloads were pulled back over the internet and hash
byte-identical to the gated artifacts (linux 0d1aa15c…, windows d911ad17…). **THE
STANDING STATE: watch the first public issues; repo topics still unset (suggestions
in §7); macOS is milestone C (hardware-blocked); performance parked with no lead
≥0.5 ms; RT parked; Case West inherits everything via docs/reusability.md.** The KB/M struggle prompt was
...
and all we'll switch to something else then performance."* ~~**PERFORMANCE IS PARKED**~~ — **RESUMED, part 87** — having
...
Where the port is, as of 2026-09-12, late afternoon (**PART 118 — EACH PIPELINE STAGE ON ITS
OWN CORE, AND THE MAIN THREAD'S BOUND NAMED.** The operator's order (*"reduce the main
thread ms ... by splitting some part you can on other core"*, deadline 16:00 EST);
`docs/perf-plan-part118.md` §1/§2/§4/§5 the record; `docs/part118-kickoff.md` THE LIVE
HAND-OFF; gotchas 570-573):

* **The PMU on the guest's Main Thread, with and without our renderer** (§1): the same
  instruction stream at IPC 1.29 vs 1.50 — 17% more cycles with the renderer. A third of
  it is L3 eviction (3,700 more DRAM fills a frame); the rest was PLACEMENT: a 4 Hz
  census caught the Main Thread on the SMT sibling of one of our saturated threads 29% of
  the time, migrating on nearly every one of its ~11 wakes a frame. The thread itself is
  pointer-chasing the title's heap (31k demand DRAM fills a frame, the hot instructions
  all consumers of a guest load, flat) — nothing of ours moves that.
* **The wait census by CALLER (`CZ_WAIT_CALLERS=1`)** named the Main Thread's waits: one
  whole-frame wait on the DRAW THREAD (`sub_827CC6A8`), one frame-boundary join. **The
  frame is a three-stage pipeline within 5% of itself** — Main (7.9 CPU + 1.3 waits),
  Draw Thread (5.8 + 3 on our fence), `cz-draw` (8.5-8.8, 95% of a core) — and a saving
  on one stage reappears as waiting on the next.
* **Havok's pool (`CZ_HAVOK_WORKERS`, the title hard-codes 2 for the 360): 4 workers
  measured Main CPU −0.21 and Main WAITS +0.5 (it parks for the workers' tail instead of
  finishing it), wall +0.46 — KILLED, stock default** (gotcha 573). The guard's
  prefetchnta (`CZ_VK_GUARD_NTA=1`): a null.
* **THREAD PLACEMENT SHIPPED ON BY DEFAULT from eight physical cores with SMT
  (`CZ_GUEST_PIN`, mode 2; `=0` the control)**: the Main Thread, the Draw Thread,
  `cz-pump` and `cz-draw` each on a physical core of its own with the SMT sibling kept
  empty, everything else confined to the other four; a `/proc/self/task` sweep catches
  inherited masks (the first build without it ran the Havok workers on the Main Thread's
  core: 25 fps, gotcha 571). **Three runs an arm, matched bands: `cz-draw` −0.52, Main
  −0.35, Draw −0.58 ms/frame, every column monotone; p99 12.0 -> 11.0; the A/B/A on the
...
AND THE FRAME'S BOUND HAS MOVED TO THE GUEST.** The operator's second unattended order of
the day (*"try to see if you can separate the pm4 pump on multiple core"*);
`docs/perf-plan-part117.md` §0 and §4 the record; `docs/part117-kickoff.md` was the live
hand-off until part 118 that evening; gotchas 565-569):

* **The PMU said what the pump is before anything was built** (`tools/part117_memprobe.sh`):
  IPC 1.61, 0.7 GB/s from DRAM, 0.47% of loads from RAM — an INSTRUCTION-bound serial
  thread, not the "bytes, machine-wide" bound part 111 named. Gotcha 551's mechanism is
  retracted in part (its observation stands: relocating the zero recovered nothing, because
  the pump re-touched every line). Two instruction streams on two cores overlap.
* **`gpu/pump_split.{h,cpp}` — the walk stays on `cz-pump`; draws, resolves, the swap,
  every guest-memory store, the INTERRUPT packets and first-sight shader binds execute in
  stream order on `cz-draw` from ONE ordered stream, against a replica register file.**
  Three rules: stores are deferred behind the draws they follow (the UP ring); interrupts
  are delivered by the pump thread at D's position; a WAIT on a word our pending store
...
* **The guest is the bound now**: Main Thread 8.1-8.4 ms CPU (+0.36 under the split —
  ...
  and render submission (part 116 §4.1) or the D3D pivot.
* `MADV_HUGEPAGE` on the guest map (the views need root's `shmem_enabled=advise`;
