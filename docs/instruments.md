...
CZ_VK_PERSIST_MB=N   where the CROSS-FRAME STREAM STORE STARTS, in MB. **The default moved
                   ...
                   frame — 71.7 ms for a 256 MB step** (waits 13.6 + allocate/map 42.9 +
                   free-old 15.2). The operator's part-79 session grew twice and both
...
## Part 106 — the GPU decomposition arms (the 60 fps / 1080p / GTX 1060 target)

Part 106 asked what the title's own passes — 72-82% of the device frame at 1080p — are
MADE OF, a question no earlier part had put to the GPU. Five same-binary arms answer it,
all off by default, all printing an engagement line, and every one of the three "garbage
picture" arms is a MEASUREMENT ARM in `docs/measurement.md`'s sense: it changes the work,
so its picture is inadmissible and its GPU column is the whole result.

| variable | what it does | reads as |
|---|---|---|
| `CZ_VK_GPU_STATS=1` | a `VK_QUERY_TYPE_PIPELINE_STATISTICS` query around every render scope, accumulated per pass class beside the timing split (needs `CZ_VK_GPU_PASSES=1`; the device must have `pipelineStatisticsQuery`, now an optional row of the feature table). Prints IA primitives, VS invocations, clipped primitives, FS invocations per frame and **FS invocations per visible pixel — the overdraw factor**. **Forces the serial recorder** (`parallel record OFF: CZ_VK_GPU_STATS is set…`): a query cannot span the parallel recorder's worker chunks. The GPU work is identical in both recorders, so the census is honest about the frame; its WALL time is the part-88 recorder's and must not be quoted | the frame's geometry and fragment population |
| `CZ_VK_NULL_PS=1` | every translated pixel shader replaced by `tools/null_ps.hlsl`, a fragment stage that does nothing (embedded as `runtime/gpu/null_ps_spv.h`, generated by `tools/gen_null_ps_shader.sh`). Vertex shader, vertex input, depth, MSAA, raster all unchanged. **Know what the driver does with it**: with no consumer, the pipeline link strips every vertex OUTPUT and the arithmetic behind it, so this arm also removes most of the vertex shader — it measured CHEAPER than `SCISSOR_1PX` at the crowd (3.2 vs 5.3 ms in the big passes), which is that stripping, not a contradiction | everything but pixel shading (and but interpolant math) |
| `CZ_VK_SCISSOR_1PX=1` | every draw's scissor collapsed to 1x1 at its own origin. Vertex shading, assembly, clipping, every bind and the per-draw cost stay; no fragment is ever shaded. A global read once at init, not a per-draw static (gotcha 453) | everything but the pixels |
| `CZ_VK_TRI1=1` | every draw issues at most its first primitive (3 indices; the expanded rect/quad lists too). Every bind, push constant, pipeline switch and pass stays; the vertex/index FETCH and the vertex shading go | the per-draw front-end cost |
| `CZ_VK_VRAM_STORE=1` | the SPLIT of part 73's refuted `CZ_VK_VRAM_STREAMS`: only the cross-frame stream store goes to a `DEVICE_LOCAL \| HOST_VISIBLE` heap; the per-frame arena (the ~8 KB of constants a draw writes) stays in RAM. The store is written ~0.2 MB/frame and read ~100 MB/frame by the vertex fetch, which is the shape VRAM is for; the arena is the opposite shape, which is why the combined arm lost wall time (gotcha 363). Prints `cross-frame stream store: N MB in VIDEO MEMORY` when engaged | a candidate MODE, measured by its GPU column first |

**`pass: shadow cascade`** is a new class in the GPU split: any render scope whose draws
bound the 1040-pitch shadow surface (`IsShadowSurface`), whatever its draw count.

**The pass EXTENT CENSUS was blind from part 89 to part 106.** Under parallel record the
per-draw extent read `R->bound.scissor`, which the capture path never writes, so it read
0x0 on every draw and the census printed no rows for every run in between — an
instrument that looked like an empty table. It reads the draw's own scissor now; any
extent claim recorded between parts 89 and 105 was made on a run that could not have
produced one.

Reading the arms together (crowd, 1080p 2x MSAA, this box; `docs/perf-plan-part106.md`
...
(thread names)     as of part 108 the audio threads are named `cz-xma-decode` and
